...
export function TransactionsProvider({ children }: TransactionsProviderProps): JSX.Element {
        ...
        >
          <View style={[styles.container, { backgroundColor: `${cardColor}`, }]}>
            ...
            <View style={[styles.titleContainer]}>
              <Text style={[styles.text]} numberOfLines={1}>{title ? title : "No title"}</Text>
            </View>
          ...
          >
            ...
            <Text
              style={[
                styles.text,
                {
                  color: `${type === "Income" || type === "To Receive" ? "#70B657" : "#D17777"}`,
                  marginBottom: 20,
                }
              ]}
              numberOfLines={1}
            ...
            <Text style={[styles.text, { fontFamily: "Poppins-Medium" }]} numberOfLines={1}>{title ? title : "No title"}</Text>
            <Text
              style={[
                styles.text,
                {
  ...
  function deleteTransaction({ id, type }: ITransaction): void {
    if (type === "Income" || type === "To Receive") {
      ...
      <>{
        categories.length > 0
          ...
          : <View style={styles.noExpensesContainer}>
            <Text style={[styles.text, { fontSize: 16 }]}>You don't have expenses.</Text>
          </View>
      }</>
    );
  }
  ...
  async function storeWalletSavings(): Promise<void> {
    ...
    setIncomes(prev => prev.filter(item => item.type !== "Income"));
    setExpenses(prev => prev.filter(item => item.type !== "Expense"));
  }
  ...
  function renderToReceiveAndDebts(): void {
    ...
    for (let i = 0; i < incomes.length; i++) {
      if (incomes[i].type === "To Receive") receiveAndDebts.push(incomes[i]);
    }
    for (let i = 0; i < expenses.length; i++) {
      if (expenses[i].type === "Debt") receiveAndDebts.push(expenses[i]);
    }
  ...
  function renderIncomesAmount(): void {
    ...
    incomes.map(income => {
      income.type === "Income" ? counter = counter + income.amount : {};
    });
  ...
  function renderExpensesAmount(): void {
    ...
    expenses.map(expense => {
      expense.type === "Expense" ? counter = counter + expense.amount : {};
    });
  ...
  function renderReceiveAmount(): void {
    let counter: number = 0;
    incomes.map(income => {
      income.type === "To Receive" ? counter = counter + income.amount : {};
    });
  ...
  function renderDebtsAmount(): void {
    let counter: number = 0;
    expenses.map(expense => {
      expense.type === "Debt" ? counter = counter + expense.amount : {};
    });
